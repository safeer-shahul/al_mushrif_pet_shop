...
import { FaTachometerAlt, FaBoxOpen, FaThList, FaTags, FaFilter, FaShoppingBag, FaFileAlt, FaUsers, FaSignOutAlt } from 'react-icons/fa'; // Added required icons
...
const adminMenuItems = [
  ...
  { name: 'Categories', href: '/mushrif-admin/categories', icon: FaThList },
...
const AdminSidebar: React.FC = () => {
  ...
  return (
    // Sidebar: Dark Primary Background, Fixed, Visible on Large Screens
    <div className="fixed inset-y-0 left-0 z-20 w-64 overflow-y-auto transition duration-300 transform 
      ...
      {/* Logo Area (Lighter primary shade for contrast) */}
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        {adminMenuItems.map((item) => {
          ...
          const Icon = item.icon; // Component from react-icons
          ...
          return (
            <Link 
              ...
              className={`flex items-center px-4 py-2 text-sm font-medium rounded-lg transition duration-200 
                ${
                  isActive
                    // Active: Light background, primary text
