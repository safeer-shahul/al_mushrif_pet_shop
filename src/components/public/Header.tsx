...
const Header: React.FC = () => {
    ...
    // 💡 REMOVED: const [isLoginModalToOpen, setIsLoginModalToOpen] = useState(false); 
    ...
    const handleLoginClick = () => {
        ...
        // 💡 Use openLoginModal from context
        openLoginModal();
    }
    ...
    const renderMegaMenu = (rootCat: RootCategory) => {
        ...
        return (
            <div className="fixed top-[117px] left-0 right-0 bg-white mr-8 ml-8 border-r border-b border-l border-gray-300 z-50">
                        ...
                        {l1SubCats.map(l1Cat => (
                            <div key={l1Cat.id} className="space-y-4">
                                <Link
                                    ...
                                    className="text-sm font-bold text-gray-900 hover:text-[var(--color-primary,#003a8c)] block uppercase tracking-wide"
                                >
                                    {l1Cat.sub_cat_name}
                                </Link>
                                        ...
                                        l1Cat.children.map(l2Cat => (
                                            <li key={l2Cat.id}>
                                                <Link
                                                    ...
                                                    onClick={handleNavLinkClick}
                                                    className="text-sm text-gray-600 hover:text-[var(--color-primary,#003a8c)] transition-colors block"
                                                >
                                                    {l2Cat.sub_cat_name}
                                                </Link>
                                            </li>
    ...
    const renderMobileMenu = () => {
        ...
        return (
            <div className="fixed inset-0 bg-black/50 z-50 lg:hidden" onClick={() => setIsMenuOpen(false)}>
                <div className="fixed top-0 left-0 h-full w-[85%] max-w-sm bg-white overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                    {/* Mobile Menu Header */}
                    ...
                    <div className="p-4 border-b border-gray-200">
                        <form onSubmit={handleSearchSubmit} className="flex">
                            <input
                                ...
                                className="flex-1 py-2 px-3 text-sm border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-[var(--color-primary,#003a8c)] bg-gray-50"
                            />
                            <button type="submit" className="bg-[var(--color-primary,#003a8c)] text-white px-4 rounded-r-md hover:bg-[var(--color-primary-light,#006ef5)]">
                        ...
                        {isAuthenticated ? (
                            <div className="space-y-3">
                                <div className="flex items-center space-x-3">
                                    <div className="w-10 h-10 rounded-full bg-[var(--color-primary,#003a8c)] text-white flex items-center justify-center font-semibold">
                                ...
                                <Link
                                    href="/user/profile"
                                    onClick={handleNavLinkClick}
                                    className="flex items-center space-x-2 text-sm text-gray-700 hover:text-[var(--color-primary,#003a8c)] py-2"
                                >
                                ...
                                <Link
                                    href="/user/wishlist"
                                    onClick={handleNavLinkClick}
                                    className="flex items-center justify-between py-2 text-sm text-gray-700 hover:text-[var(--color-primary,#003a8c)]"
                                >
                            ...
                            <>
                                <button
                                    ...
                                    className="w-full py-2.5 px-4 rounded-md border-2 border-[var(--color-primary,#003a8c)] text-[var(--color-primary,#003a8c)] font-semibold flex items-center justify-center hover:bg-[var(--color-primary,#003a8c)] hover:text-white transition-colors mb-3"
                                >
                    ...
                    <div className="py-2">
                        {navLoading ? (
                            <div className="flex items-center justify-center py-4">
                                <FaSpinner className="animate-spin text-[var(--color-primary,#003a8c)]" />
                            </div>
                        ) : (
                            <>
                                {rootCategories.map(rootCat => (
                                    <div key={rootCat.id} className="border-b border-gray-100">
                                        <Link
                                            ...
                                            onClick={handleNavLinkClick}
                                            className="block px-4 py-3 text-sm font-medium text-slate-800 hover:text-[var(--color-primary,#003a8c)] hover:bg-gray-50"
                                        >
                                            {rootCat.cat_name}
                                        </Link>
                                    </div>
    ...
    return (
        <header className="sticky top-0 z-50 bg-white shadow-sm">
            ...
            <div className="shadow-sm" style={{ backgroundColor: 'var(--color-primary, #003a8c)' }}>
                <div className="container mx-auto px-4 lg:px-8 h-16 flex items-center justify-between gap-4">
                    ...
                    <form onSubmit={handleSearchSubmit} className="hidden lg:flex flex-1 max-w-2xl">
                        <div className="relative w-full">
                            ...
                            <button
                                type="submit"
                                className="absolute right-0 top-0 h-full w-12 text-gray-600 hover:text-[var(--color-primary,#003a8c)] transition-colors flex items-center justify-center"
                            >
                                <FaSearch className="w-4 h-4" />
                            </button>
                        </div>
                    ...
                    <div className="flex items-center space-x-2 flex-shrink-0">
                        ...
                        >
                            ...
                            {isMounted && cartCount > 0 && (
                                <span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-bold text-[var(--color-primary,#003a8c)] bg-white rounded-full">
                ...
                <div className="container mx-auto px-8">
                    {navLoading ? (
                        ...
                        <ul className="flex justify-start space-x-1">
                            {rootCategories.map(rootCat => {
                                const subCategoryCount = (rootCat.subCategories || rootCat.sub_categories)?.length || 0;
                                const isDropdown = subCategoryCount > 0;

                                return (
                                    <li
                                        key={rootCat.id}
                                        className="relative group"
                                        onMouseEnter={() => isDropdown && setActiveRootCategory(rootCat)}
                                        onMouseLeave={() => isDropdown && setActiveRootCategory(null)}
                                    >
                                        <Link
                                            href={`/products?category_id=${rootCat.id}`}
                                            onClick={handleNavLinkClick}
                                            className={`text-sm font-bold uppercase py-4 px-5 block transition-colors ${(isDropdown && activeRootCategory?.id === rootCat.id)
                                                    ? 'text-[var(--color-primary,#003a8c)]'
                                                    : 'text-gray-800 hover:text-[var(--color-primary,#003a8c)]'
                                                }`}
                                        >
                                            {rootCat.cat_name}
                                        </Link>

                                        {isDropdown && activeRootCategory?.id === rootCat.id && renderMegaMenu(rootCat)}
                                    </li>
                                );
                            })}
                            ...
                            {STATIC_MENU_ITEMS.map(item => (
                                <li key={item.name} className="relative">
                                    <Link
                                        href={item.href}
                                        className="text-sm font-bold uppercase py-4 px-5 block text-gray-800 hover:text-[var(--color-primary,#003a8c)] transition-colors"
                                    >
                                        {item.name}
                                    </Link>
                                </li>
            ...
            {/* Modals */}
            <LoginModal
                // 💡 Use Modal Context state and handler
