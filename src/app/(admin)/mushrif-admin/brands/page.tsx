// src/app/(admin)/mushrif-admin/brands/page.tsx
'use client';

import React, { useState, useEffect, useCallback, useRef } from 'react';
import Link from 'next/link';
import LoadingSpinner from '@/components/ui/LoadingSpinner';
import { FaPlus, FaEdit, FaTrash, FaSync, FaImage, FaTags } from 'react-icons/fa';
import { Brand } from '@/types/brand';
import { useBrandService } from '@/services/admin/brandService';

const BrandListPage: React.FC = () => {
    const { fetchAllBrands, deleteBrand, getStorageUrl } = useBrandService();
    
    const [brands, setBrands] = useState<Brand[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const hasFetchedRef = useRef(false);

    // Load function
    const loadBrands = useCallback(async () => {
        if (loading && hasFetchedRef.current) return;
        
        setLoading(true);
        setError(null);
        
        try {
            const fetchedBrands = await fetchAllBrands(); 
            setBrands(fetchedBrands);
            setError(null);
            hasFetchedRef.current = true;
        } catch (err: any) {
            setError(err.message || 'Failed to load brands');
            console.error('Error loading brands:', err);
            setBrands([]);
        } finally {
            setLoading(false);
        }
    }, [fetchAllBrands, loading]); 

    // Load on mount only
    useEffect(() => {
        if (!hasFetchedRef.current) {
            loadBrands();
        }
    }, [loadBrands]);

    const handleDelete = async (id: string, name: string) => {
        if (!window.confirm(`Are you sure you want to delete the Brand: ${name}? This action cannot be undone.`)) return;
        
        try {
            setError(null);
            await deleteBrand(id);
            
            const toast = document.createElement('div');
            toast.className = 'fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg z-50';
            toast.textContent = 'Brand deleted successfully.';
            document.body.appendChild(toast);
            setTimeout(() => document.body.removeChild(toast), 3000);
            
            // Reload the list
            hasFetchedRef.current = false;
            await loadBrands();
        } catch (err: any) {
            setError(err.message || 'Failed to delete brand. Check for associated products.');
            console.error('Delete error:', err);
        }
    };

    const handleManualRefresh = () => {
        hasFetchedRef.current = false;
        loadBrands();
    };

    if (loading && brands.length === 0 && !error) {
        return (
            <div className="flex items-center justify-center h-[50vh]">
                <LoadingSpinner />
            </div>
        );
    }

    return (
        <div className="space-y-6">
            {/* Header section with actions */}
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">Brand Management</h1>
                    <p className="text-gray-500 mt-1">Manage all product brands and their status</p>
                </div>
                <div className="flex gap-2">
                    <button 
                        onClick={handleManualRefresh}
                        disabled={loading}
                        className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-200 transition-colors shadow-sm disabled:opacity-50"
                    >
                        <FaSync className={`inline-block mr-2 ${loading ? 'animate-spin' : ''}`} />
                        {loading ? 'Loading...' : 'Refresh'}
                    </button>
                    <Link href="/mushrif-admin/brands/create" passHref>
                        <button className="flex items-center px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-500 rounded-lg hover:from-blue-700 hover:to-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors shadow-sm">
                            <FaPlus className="mr-2" /> Add New Brand
                        </button>
                    </Link>
                </div>
            </div>

            {/* Error alert */}
            {error && (
                <div className="p-4 bg-red-50 border-l-4 border-red-500 rounded-lg">
                    <p className="text-sm text-red-700">{error}</p>
                </div>
            )}

            {/* Data table */}
            {!error && brands.length > 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead>
                                <tr className="bg-gray-50">
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Logo</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Description</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200">
                                {brands.map((brand, index) => {
                                    const logoUrl = getStorageUrl(brand.brand_logo);
                                    return (
                                        <tr key={brand.brand_id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                {logoUrl ? (
                                                    <div className="w-16 h-8 rounded-md overflow-hidden border border-gray-200 flex items-center justify-center bg-white">
                                                        <img 
                                                            src={logoUrl} 
                                                            alt={brand.brand_name} 
                                                            className="max-w-full max-h-full object-contain p-1"
                                                        />
                                                    </div>
                                                ) : (
                                                    <div className="w-16 h-8 rounded-md bg-gray-200 flex items-center justify-center">
                                                        <FaImage className="text-gray-400 w-4 h-4" />
                                                    </div>
                                                )}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-gray-900">{brand.brand_name}</div>
                                            </td>
                                            <td className="px-6 py-4 max-w-xs">
                                                <div className="text-sm text-gray-500 truncate">{brand.brand_description || 'No description'}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${brand.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                                    {brand.is_active ? 'Active' : 'Inactive'}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                                <Link href={`/mushrif-admin/brands/${brand.brand_id}`}>
                                                    <button className="p-1.5 bg-blue-50 text-blue-700 rounded-md hover:bg-blue-100 transition-colors">
                                                        <FaEdit />
                                                    </button>
                                                </Link>
                                                <button 
                                                    onClick={() => handleDelete(brand.brand_id, brand.brand_name)}
                                                    className="p-1.5 bg-red-50 text-red-700 rounded-md hover:bg-red-100 transition-colors"
                                                    disabled={loading}
                                                >
                                                    <FaTrash />
                                                </button>
                                            </td>
                                        </tr>
                                    );
                                })}
                            </tbody>
                        </table>
                    </div>
                </div>
            ) : (
                /* Empty state */
                !loading && (
                    <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
                        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-blue-50 mb-4">
                            <FaTags className="h-8 w-8 text-blue-500" />
                        </div>
                        <h3 className="text-lg font-medium text-gray-900">No Brands Found</h3>
                        <p className="mt-2 text-gray-500 max-w-sm mx-auto">Create your first brand to assign to products.</p>
                        <div className="mt-6">
                            <Link href="/mushrif-admin/brands/create" passHref>
                                <button className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-500 rounded-lg hover:from-blue-700 hover:to-blue-600 shadow-sm">
                                    <FaPlus className="mr-2" /> Create First Brand
                                </button>
                            </Link>
                        </div>
                    </div>
                )
            )}
        </div>
    );
};

export default BrandListPage;