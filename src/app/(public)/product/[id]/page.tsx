...
import React, { useState, useEffect, useCallback, useMemo, useRef } from 'react'; // ⬅️ IMPORT useRef
import { useParams } from 'next/navigation';
// Icons needed for the description and other components
...
// We'll approximate '3 or more lines' for a typical font size to be around 144px (max-h-36 in Tailwind)
...
const ProductDetailPage: React.FC = () => {
    ...
    // 🚀 NEW: State to determine if the "See More" button is needed
    const [needsExpansionButton, setNeedsExpansionButton] = useState(false);
    
    // 🚀 NEW: Ref for measuring the description content
    ...
    // 🚀 NEW: Effect to check if "See More" is needed
    useEffect(() => {
        // Only run if the product data and description ref are available
        if (product?.description && descriptionRef.current) {
            // Check if the content's actual height (scrollHeight) is greater than the visible collapsed height
            const isTallerThanCollapsed = descriptionRef.current.scrollHeight > COLLAPSED_HEIGHT_PIXELS;
            setNeedsExpansionButton(isTallerThanCollapsed);
        }
        // Dependency array includes product.description to re-evaluate if content changes
    }, [product?.description, isDescriptionExpanded, loading]); // Added isDescriptionExpanded to re-measure after expansion/collapse
    ...
    // Condition to check if variants should be shown as options.
        ...
        <div className="container mx-auto px-4 py-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 bg-white p-6 rounded-xl shadow-2xl border border-gray-100">
                ...
                <div className="space-y-5">
                    ...
                    {/* Variant Selector - HIDDEN if it's a base product */}
                    {shouldShowVariantOptions && product.variants && product.variants.length > 0 && (
                    ...
                    {product.description && (
                        <div className="mt-4 pt-3 border-t border-gray-100 space-y-3">
                            <h3 className="text-lg font-bold text-slate-800 flex items-center">
                                <FaListAlt className='mr-2 w-4 h-4 text-gray-600' /> Product Details
                            </h3>
                            ...
                            >
                                ...
                                />
                                {/* Fade effect at the bottom when collapsed AND needs the button */}
                                {!isDescriptionExpanded && needsExpansionButton && (
                                    <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-white to-transparent pointer-events-none"></div>
                                )}
                            </div>

                            {/* "See More" Button - Only render if needsExpansionButton is true */}
                            {needsExpansionButton && (
                                <button
                                    onClick={() => setIsDescriptionExpanded(prev => !prev)}
                                    className="flex items-center text-sm font-semibold transition-colors hover:text-gray-700"
                                    style={{ color: PRIMARY_COLOR }}
                                >
                                    {isDescriptionExpanded ? (
                                        <>
                                            <FaChevronUp className='mr-2 w-3 h-3' />
                                            See Less
                                        </>
                                    ) : (
                                        <>
                                            <FaChevronDown className='mr-2 w-3 h-3' />
                                            See More
                                        </>
                                    )}
                                </button>
